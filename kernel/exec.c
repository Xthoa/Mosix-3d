...
void ePeMakeActiveDllList(Process* p, ImageNtHeaders64* inh, u64 base){
    ImageDataDirectory* iddi = inh->OptionalHeader.DataDirectory + IMAGE_DIRECTORY_ENTRY_IMPORT;
    u32 dllcnt = iddi->Size / 20;
    p->dlls.dlls = kheap_alloc(sizeof(ActivePedll) * (dllcnt + 8));
    p->dlls.count = 0;
    p->dlls.base = base;
    p->dlls.idata = iddi->VirtualAddress;
    init_spinlock(&p->dlls.lock);
}
Process* RunPe64(File *f){
    ...
    ePeMakeActiveDllList(p, inh, base);
    kheap_free(inh);
...
ActivePedll* MapPedll(Process* p, Pedll* dll){
    u64 top = PEDLL_TOP;
    acquire_spin(&p->dlls.lock);
    if(p->dlls.count) top = p->dlls.dlls[p->dlls.count - 1].base;
    ActivePedll* adll = p->dlls.dlls + p->dlls.count++;
    u64 base = top - dll->imgsize;
    adll->base = base;
    release_spin(&p->dlls.lock);
    adll->dll = dll;
    for(int i = 0; i < dll->seccnt; i++){
...
void LoadPedllsByImportTable(Process* self){
    ActivePedllList* dl = &self->dlls;
    ImageImportDescriptor* iid = dl->base + dl->idata;
    for(; iid->Characteristics && iid->Name; iid ++){
		char* name = dl->base + iid->Name;
        Pedll* dll = LocatePedll(name);
        ActivePedll* adll = MapPedll(self, dll);
		ImageThunkData64* t = dl->base + iid->FirstThunk;
		for(; t->Ordinal; t ++){
			if(t->AddressOfData >> 31) t->Function = GetProcAddrByOrd(self, adll, t->Ordinal);
			else t->Function = GetProcAddrByName(self, adll, dl->base + t->AddressOfData + 2);
		}
